...
const EventForm: React.FC<EventFormProps> = ({ onEventGenerated, onError }) => {
  ...
  const handleTextSubmit = async () => {
    ...
    onError('');
    try {
      const eventData = await extractEventFromText(textInput);
      onEventGenerated(eventData);
    } catch (err) {
      onError("Failed to understand the event. Please try being more specific.");
    } finally {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        // We use Gemini to "clean up" the ICS or understand it if it's complex
        // Passing the raw ICS text to Gemini is robust because Gemini can handle the format parsing
        const eventData = await extractEventFromText(`Parse this ICS file content into a single structured event: \n\n ${content}`);
        onEventGenerated(eventData);
      } catch (err) {
        onError("Could not parse the file. Is it a valid calendar file?");
      } finally {
        setIsProcessing(false);
        // Reset file input
      ...
      <div className="flex mb-6 border-b border-slate-100">
        <button
          className={`flex-1 pb-3 text-sm font-medium transition-colors relative ${
            inputMode === 'text' ? 'text-primary-600' : 'text-slate-400 hover:text-slate-600'
          }`}
        ...
        <button
          className={`flex-1 pb-3 text-sm font-medium transition-colors relative ${
            inputMode === 'file' ? 'text-primary-600' : 'text-slate-400 hover:text-slate-600'
          }`}
        ...
        {inputMode === 'text' ? (
          <div className="flex flex-col h-full animate-fade-in">
            ...
            <div className="mt-4 flex justify-end">
              ...
              >
                {isProcessing ? <Spinner /> : <i data-feather="zap" className="w-4 h-4"></i>}
                {isProcessing ? 'Analyzing...' : 'Create Event'}
              </button>
            </div>
          </div>
        ) : (
          <div className="h-full flex flex-col justify-center items-center border-2 border-dashed border-slate-200 rounded-xl p-8 bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer relative group animate-fade-in">
             <input
              ...
              <div className="bg-white p-4 rounded-full shadow-sm mb-4 group-hover:scale-110 transition-transform duration-300">
                 <i data-feather="upload-cloud" className="w-8 h-8 text-primary-500"></i>
              </div>
              <p className="text-sm font-medium text-slate-700">
                 Click to upload or drag & drop
              </p>
              <p className="text-xs text-slate-400 mt-1">
                Supports .ics or .txt files
              </p>
              {isProcessing && (
